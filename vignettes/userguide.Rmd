...
this function takes a vector of time series data quantifying a transient (in this case from a video of calcium flux in a cardiomyocyte, **"ca_flux"**), and returns the location where each peak is located. 
...
This function takes a vector of time series data quantifying a transient, and returns  the height of each peak (the y value, or the maximum deviation from the established baseline). 
...
This function takes a vector of time series data quantifying a transient, and extracts all the curves from the trace data. 
...
## Find pulse velocity with `max_velocities()`

This function returns the maximum velocity of the transient, or the maximum slope (positive and negative) of the peaks. The function takes a vector of time series data quantifying a transient, and returns both the locations of the maximum up (x.up) and down (x.down) velocities, as well as the values for the maximum up and down velocities. 
...
This function divides the maximum transient velocity of each peak by the baseline value at the point where the transient reaches its maximum velocity. 
...
## Find the area under a specific peak with `integrate_peak()`

This function returns the area of a specified peak in a transient. The function takes a vector of time series data quantifying a transient, as well as a specified peak number. 

```{r}
integrate_peak(ca_flux$Mean1, 1)

integrate_peak(ca_flux$Mean2, 14)
```

## Find the area under each peak with `peak_integrals()`

This function simply returns the area under every peak in a transient. 
